-- Clear all rows tied to access code STUDY2026 (test / lab channel).
-- Tables: participants, conversations, survey_responses
-- Run in Supabase SQL Editor. Review the preview counts before DELETEs.
...
FROM participants
WHERE access_code = 'STUDY2026'
UNION ALL
SELECT 'conversations', COUNT(*)
FROM conversations
WHERE participant_id IN (SELECT id FROM participants WHERE access_code = 'STUDY2026')
UNION ALL
...
WHERE participant_id IN (
  SELECT id FROM participants WHERE access_code = 'STUDY2026'
);
